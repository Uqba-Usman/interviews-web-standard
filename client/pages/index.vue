<template>    
        ...
        <div class="modal-content">
          ...
          <div v-if="showTagSelector" class="form-group mt-3">
            ...
            >
              ...
              <!-- No tags found and option to create new tag -->
              <template #noResult>
                No tags found.
  ...
  export default {
    ...
    methods: {
      goToTaskDetail(taskId) {
        this.$router.push(`/tasks/show/${taskId}`); // Navigate to task detail page
      },
      fetchTaskList() {
        this.loading = true;
        getTasks()
      ...
      fetchTags() {
        getTags()
          .then((response) => (this.tags = response.data.$values || [])) // Fetch tags from service
          .catch(() =>
      ...
      handleSaveTags() {
        // Save selected tags for the task
        this.isSaving = true;
  ...
  <style scoped>
  /* Styles for task manager components */
  .task-manager-title {
    font-size: 28px;
    color: #052e82;
    font-weight: bold;
